...
export default (
  ...
  ({ options, store }) => {
    const persistForEach = (
      ...
      persist.forEach(
        ({ key, includes = Object.keys(store.$state), excludes = [], setter = SETTER, getter = GETTER }, index) => {
    ...
    store.$persist = () =>
      persistForEach(({ key, paths, setter }) => setter?.call(store, key, deepPick(store.$state, ...paths)));
